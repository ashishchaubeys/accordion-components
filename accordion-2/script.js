...
class CircleExpand extends HTMLElement {
  ...
  const summary = document.querySelectorAll('.accordion-heading');
  ...
  // details element
  const details_element = document.querySelectorAll('.accordion-wrapper');

  for (let i = 0; i < details_element.length; i++) {
    details_element[i].isTransitioning = false;

    const summary_elememnt = details_element[i].querySelector('.accordion-heading');
